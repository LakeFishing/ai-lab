...
for otn in range(1, ws.max_row + 1):
    otn_list_t = num_list[otn - 1][0]
    otn_list.append(otn_list_t)

x_train = np.array(row_list)
y_train = np.array(otn_list)
x_train = x_train.reshape(ws.max_row, 28, 28, 1)
y_train = y_train.reshape(ws.max_row)
x_train = x_train.astype('float32')
y_train = y_train.astype('uint8')

x_test = np.array(row_list)
y_test = np.array(otn_list)
x_test = x_test.reshape(ws.max_row, 28, 28, 1)
y_test = y_test.reshape(ws.max_row)
...
# model.save('./t1h5/CNN01.h5')
model.save('./test/t1h5/CNN01.h5')
del model